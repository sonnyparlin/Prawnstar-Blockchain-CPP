...
\item 
static std\+::string \mbox{\hyperlink{class_node_a204be29a96288955c69ce5ee3942954e}{get\+Time\+Str}} ()
\item 
...
The node is the interface for both the API and the P2P network to interact with the blockchain. The node is where transactions and blocks are handled whether they come from the API or the P2P network. 
...
This is where new blocks are initiated for this forger. Return the new block forger based on the proof of work algorithm.

if there are more than 100 transactions in the transaction pool assume we are having connection problems with the current staker\textquotesingle{}s node and remove that staker from the stakers vector.

100 transactions in the transaction pool means there is a clog in the system, such a clog will be in the form of a node that has staked prawn but the node is unreachable and so transactions are piling up in the transaction pool. So let\textquotesingle{}s get the address of this staker, return the stake back to the staker\textquotesingle{}s wallet and then remove the staker from the list of valid stakers. Then re-\/run the proof of stake lottery to find a new staker and proceed with forging the new block.

At 500 transactions in the pool, we know it\textquotesingle{}s time to clean up. There should never be this many transactions in the transaction pool. This can only happen if the node was identified as a staker but wasn\textquotesingle{}t able to \mbox{\hyperlink{class_node_ac08fe080956d22ded0226fd3b47a4fdc}{forge()}} new blocks, perhaps due to network connectivity issues or some other reason. Either way, we can safely assume these are dead transactions or they\textquotesingle{}ve been processed already.

Okay now that we\textquotesingle{}ve removed the broken staker, let\textquotesingle{}s try to pick a new staker.\mbox{\Hypertarget{class_node_ab3291bce63c6c99090e49abcbb4c29b5}\label{class_node_ab3291bce63c6c99090e49abcbb4c29b5}} 
...
std\+::string
\end{DoxyReturn}
Return \mbox{\hyperlink{class_node}{Node}}\textquotesingle{}s peer to peer server id \mbox{\Hypertarget{class_node_a204be29a96288955c69ce5ee3942954e}\label{class_node_a204be29a96288955c69ce5ee3942954e}} 
\index{Node@{Node}!getTimeStr@{getTimeStr}}
\index{getTimeStr@{getTimeStr}!Node@{Node}}
\doxysubsubsection{\texorpdfstring{getTimeStr()}{getTimeStr()}}
{\footnotesize\ttfamily std\+::string Node\+::get\+Time\+Str (\begin{DoxyParamCaption}{ }\end{DoxyParamCaption})\hspace{0.3cm}{\ttfamily [static]}}

\begin{DoxyReturn}{Returns}
std\+::string
\end{DoxyReturn}
Return the current time as a string. \mbox{\Hypertarget{class_node_a3431dcfd8525b2eb4bc74262363d0465}\label{class_node_a3431dcfd8525b2eb4bc74262363d0465}} 
...
\hline
\end{DoxyParams}
This is where we read and rebuild the blockchain or partial blockchain after requesting blocks from the master server. How many blocks we ad is based on how many we requested. \mbox{\Hypertarget{class_node_af25a469a96f939eb8a2bde8de080a2cb}\label{class_node_af25a469a96f939eb8a2bde8de080a2cb}} 
...
\mbox{\hyperlink{class_node}{Node}}\textquotesingle{}s proof\+Of\+Stake 
